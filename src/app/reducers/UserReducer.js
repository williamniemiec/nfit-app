...
export default (state=initialState, action) => {
    ...
    switch (action.type) {
        ...
        case 'DEL_WORKOUT_DAY':
            newWorkoutDays = state.workoutDays.filter(item => item != action.payload.workoutDay)
        ...
        case 'DEL_MY_WORKOUTS':
            newMyWorkouts = state.myWorkouts.filter(item => item != action.payload.workoutId)
        ...
        case 'DELETE_DAILY_PROGRESS':
            dailyProgress = state.dailyProgress.filter((date, index) => date != action.payload.date)
